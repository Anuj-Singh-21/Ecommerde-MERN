...
const sortOptions = [
  { name: "A-Z", sort: "title", order: "asc", current: true },

  { name: "Z-A", sort: "title", order: "desc", current: false },
  {
...
export default function ProductList() {
  ...
  const handleSearchSubmit = (event) => {
    ...
    dispatch(fetchProductsBySearchAsync(searchQuery));
  };

  const handleSort = (e, option) => {
    const sort = option.sort
      ? { _sort: option.sort, _order: option.order }
      : {}; // Added check to handle A-Z sorting
    console.log({ sort });
    setSort(sort);
  };
